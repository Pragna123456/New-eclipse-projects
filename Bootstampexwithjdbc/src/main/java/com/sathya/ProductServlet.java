...
public class ProductServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		  
		...
		String proBand=request.getParameter("proBand");
		String proMade=request.getParameter("proMade");
		
		Date proMdate=Date.valueOf(request.getParameter("proMdate"));
		Date proExdate=Date.valueOf(request.getParameter("proExdate"));
	    ...
	    product.setProBand(proBand);
	    product.setProMade(proMade);
	    product.setProMdate(proMdate);
	    product.setProExdate(proExdate);
	    ...
	    ProductDao dao=new ProductDao();
			...
			if(result==1)
			{
		        ...
		        {	response.setContentType("text/html");
		        	...
		        	RequestDispatcher dispatcher = request.getRequestDispatcher("addform.html");
